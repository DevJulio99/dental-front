...
const roleOptions = ['Odontólogo', 'Asistente', 'Administrador'];
      ...
      <div className="w-full max-w-lg p-6 bg-white rounded-lg shadow-xl">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div><label htmlFor="role" className="block text-sm font-medium text-gray-700">Rol</label><select id="role" value={formData.role} onChange={handleInputChange} className="w-full px-3 py-2 mt-1 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary"><option disabled>Seleccione un rol</option>{roleOptions.map(role => (<option key={role} value={role}>{role}</option>))}</select></div>
          </div>
          <div className="flex justify-end mt-6 space-x-2">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-medium text-white rounded-md bg-primary hover:bg-blue-600">Guardar Usuario</button>
          </div>
        </form>
      </div>
    </div>
